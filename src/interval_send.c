...
void interval_send_tick( uint32_t global_ms )
{
    if( send_enable && send_list )
    {
        ...
        // Walk the array of items to send
...
{
    return interval_send_add( find_tracked_object(id), interval );
}
...
{
    return interval_send_remove( find_tracked_object(id) );
}
...
{
    return interval_send_start( find_tracked_object(id) );
}
...
{
    return interval_send_stop( find_tracked_object(id) );
}
